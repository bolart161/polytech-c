...
void task2(){
  ...
  const size_t vertices = countVertices(vector);
...
Shape readShapeData(const Line &line)
{
  ...
  if(iss.fail())
  {
    throw std::invalid_argument("Invalid points value\n");
  }

  std::string remLine;
  std::getline(iss, remLine);
  if (std::find_if(remLine.begin(), remLine.end(), [](char c) {return !std::isspace(c, std::locale()); }) != remLine.end()) {
...
{
  const int num = std::accumulate(vector.begin(), vector.end(), 0, [](int verticesNum, const Shape &shape) {
...
{
  vector.erase(std::remove_if(vector.begin(), vector.end(), [](const Shape &sh){return sh.size() == 5;}), vector.end());
}
